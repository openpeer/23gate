...
async function loadWebhooks() {
  ...
  for (const webhook of webhooks) {
    ...
    webhook.keys = []
    for (const address of sequelize.models.Webhook.parseAddressListString(
      webhook.addressList
    )) {
...
function parse(transactionReceipt, webhooks, payloadList) {
  ...
  for (const webhook of webhooks) {
    for (const key of webhook.keys) {
      ...
      logger.trace(
        `Got match on tx=${transactionReceipt.transactionHash} webhook=${webhook.id}`
      )
      ...
      for (const log of logsByTopic0AndAddress[key]) {
        ...
        if (!parsed) {
          logger.warn(
            `Couldn't parse ${log.transactionHash} for webhook ${webhook.id}`
          )
          continue
        }

        logger.trace(
          `Found eventName=${parsed.name} in tx=${transactionReceipt.transactionHash} webhook=${webhook.id}`
        )
...
function createPayloads(payloadList) {
  ...
  return payloadList.map(
    ({ eventName, log, args, transactionReceipt, webhook }) => {
...
async function savePayloadsAndDeleteTransactions(
  records,
  transactionHashList,
  c = 0
) {
  ...
  try {
    ...
    if (e.parent?.code == 'ER_LOCK_DEADLOCK') {
      if (c > 1) {
        logger.warn(
          `savePayloadsAndDeleteTransactions DEADLOCKED in ${
            Date.now() - startedAt
          }ms at try ${c}`
        )
      }
...
async function processDownloadedTransactions() {
  do {
    ...
    const transactionsList =
      await sequelize.models.Transaction.findTransactionsToParseByChainId(
        chainId,
        300
      )
    ...
    await savePayloadsAndDeleteTransactions(
      records,
      transactionsList.map((t) => t.transactionHash)
    )
