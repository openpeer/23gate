...
const WAIT_BETWEEN_NO_PAYLOADS_MS = ms('5s')
const WAIT_BETWEEN_PAYLOADS_FOUND_MS = ms('5s')
...
async function abandonHungRequests() {
  if (
    Date.now() - lastAbandonHungRequestsDeletedAt <
    EXPIRE_ABANDONED_REQUESTS_INTERVAL
  ) {
    return
  }

  await sequelize.query(
    'UPDATE Payload SET isSending = 0 WHERE isSending = 1 AND requestStartedAt <= NOW() - INTERVAL ? MINUTE',
    {
...
async function sendPayload(payload, userSecrets) {
  const secret = payload.userId
    ? userSecrets[payload.userId]
    : TEST_WEBHOOK_SIGNATURE_SECRET
  if (!secret) {
    logger.error(
      `Can't find user secret for payload id=${payload.id} userId=${payload.userId}`
    )
  ...
  const signature = sequelize.models.Payload.signPayload(
  ...
  logger.trace(
    `HTTP response payload=${payload.id} url=${payload.url} status=${
      response ? response.status : 'oops'
    }`
  )
  ...
  const responseValues = {
    responseHeaders: response
      ? serializeHeaders(response.status, response.statusText, response.headers)
      : null,
  ...
  await sequelize.models.Webhook.incrementFailedCountAndPossiblyDisableById(
    payload.webhookId
  )
}
...
async function sendPayloadListByWebhookId(payloadListByWebhookId, userSecrets) {
  const promises = Object.values(payloadListByWebhookId).map((payloadList) =>
    sendPayloadList(payloadList, userSecrets)
  )
...
async function cycleRun() {
  do {
    ...
    const records = await sequelize.models.Payload.findPayloadsToSend(
      MAX_PAYLOADS_TO_PROCESS_COUNT
    )
    if (records.length == 0) {
      if (isExiting) {
        return
